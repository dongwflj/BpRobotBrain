...
ERESULT RobotMotioningState::naviDoneEvent(string task_id, int32 state, string description) {
    ROS_INFO("RobotMotioningState::naviDoneEvent entry");
    context_->getRobotEngineObserver().onNaviDone(task_id, state, description);
